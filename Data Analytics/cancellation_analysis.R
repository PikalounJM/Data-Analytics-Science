...
length(unique(all$고객번호)) #총 취소 고객 수: 63,415명 
...
sum(all$LEADTIME==0) #당일 예약: 7,418건(1%)
sum(all$LEADTIME<=30) #30일 이내 예약 건: 369,349건(43%)
sum(all$LEADTIME>=60) #2개월 전 예약 건: 279,282건(32%)
...
sum(all$`취소일-예약일`== 0) #예약 후, 당일 취소: 190,328건
sum(all$`취소일-예약일`<= 3) #예약 후, 3일 이내 취소: 313,023건
sum(all$`취소일-예약일` <= 30) #30일 이내 취소: 554,045건
sum(all$`취소일-예약일` <= 60) #60일 이내 취소: 699,427건
sum(all$`취소일-예약일` > 60) #2개월 후 취소: 153,697건
...
sum(all$`도착일-취소일`==0) #도착일 당일 취소: 39,339건
sum(all$`도착일-취소일`<=7) #도착일 7일 전 취소: 244,149건
sum(all$`도착일-취소일`<=14) #도착일 14일 전 취소: 553,998건
sum(all$`도착일-취소일`<=21) #도착일 3주 전 취소: 646,743건
